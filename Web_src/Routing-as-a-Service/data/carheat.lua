...
local NROWS = tonumber(os.getenv("NROWS"))
local NCOLS = tonumber(os.getenv("NCOLS"))
...
-- see https://github.com/Project-OSRM/osrm-backend/wiki/Integrating-third-party-raster-data
...
function source_function ()
  ...
  if not path then
    io.write("PATH NOT FOUND -- ENV VAR not Set ?\n")
  end
  io.write("constant precision " .. constants.precision)
...
function segment_function (segment)
  ...
  if out_of_bounds == false then
    ...
    local penalty = (targetData.datum+sourceData.datum) / 2;
    
    --if distance ~= 0 and targetData.datum > 0 and sourceData.datum > 0 then
    --  penalty = elev_delta / distance
    --end

    if penalty > 1 then
      io.write("segment weight " .. segment.weight .. " result " ..  segment.weight * penalty .. " penalty " .. penalty .."\n")
      io.write("\tsegment source lon lat ".. segment.source.lon .. " ".. segment.source.lat.."\n")
      io.write("\tsegment target lon lat ".. segment.target.lon .. " ".. segment.target.lat.."\n")
      segment.weight = segment.weight * penalty
    end
  end
end
