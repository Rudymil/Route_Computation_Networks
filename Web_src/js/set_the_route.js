...
map.locate({
        ...
        markerDeparture = L.marker([e.latitude, e.longitude], {
            icon: greenIcon,
            draggable: true
...
function showContextMenu1(marker, pos, ep) {
    ...
    $("#cm_debut").click(ep, function() {
        ...
        markerDeparture = L.marker([ep.latlng.lat, ep.latlng.lng], {
            icon: greenIcon,
            draggable: true
        }).bindPopup(ep.latlng.lat + ", " + ep.latlng.lng);
        map.addLayer(markerDeparture);
    ...
    $("#cm_fin").click(ep, function() {
        ...
        markeraDestination = L.marker([ep.latlng.lat, ep.latlng.lng], {
            icon: redIcon,
            draggable: true
        }).bindPopup(ep.latlng.lat + ", " + ep.latlng.lng);
...
$("#inverse").click(function() {
    if ($("#Navigate").is(":checked")) {
        ...
        if ($("#dep").val() == "") {
            map.removeLayer(markeraDestination);
            markerDeparture = L.marker([latdep, lngdep], {
                icon: greenIcon,
                draggable: true
            }).bindPopup(latdep + ", " + lngdep);
            map.addLayer(markerDeparture);
            $("#dep").val(temp2);
            $("#dest").val("");
            markerDeparture.on("dragend", function(ev) {
            ...
            map.removeLayer(markerDeparture);
            markeraDestination = L.marker([latdest, lngdest], {
                icon: redIcon,
                draggable: true
            }).bindPopup(latdest + ", " + lngdest);
            map.addLayer(markeraDestination);
            $("#dest").val(temp1);
            $("#dep").val("");
            markeraDestination.on("dragend", function(ev) {
...
$("#godep").click(function() {
    if ($("#Navigate").is(":checked")) {
        ...
        if (lat == '' || lng == '' || lat == '.' || lng == '.' || temp1 > 1 || (!floatRegex.test(lat) && !floatRegex.test(lng)) || (isNaN(lat) && isNaN(lng)) || lat > 90 || lat < -90 || lng > 90 || lng < -90) {
            ...
            markerDeparture = L.marker([lat, lng], {
                icon: greenIcon,
                draggable: true
            }).bindPopup(lat + ", " + lng);
            map.addLayer(markerDeparture);
...
$("#godest").click(function() {
    if ($("#Navigate").is(":checked")) {
        ...
        if (lat == '' || lng == '' || lat == '.' || lng == '.' || temp1 > 1 || (!floatRegex.test(lat) && !floatRegex.test(lng)) || (isNaN(lat) && isNaN(lng)) || lat > 90 || lat < -90 || lng > 90 || lng < -90) {
            ...
            markeraDestination = L.marker([lat, lng], {
                icon: redIcon,
                draggable: true
            }).bindPopup(lat + ", " + lng);
            map.addLayer(markeraDestination);
